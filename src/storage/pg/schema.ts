/**
 * Postgres DDL schema for Strata.
 *
 * All 13+ tables from the SQLite schema, converted to Postgres syntax:
 * - AUTOINCREMENT -> GENERATED ALWAYS AS IDENTITY
 * - INSERT OR REPLACE patterns -> ON CONFLICT DO UPDATE (handled in store code)
 * - tsvector GENERATED ALWAYS AS (...) STORED columns + GIN indexes for FTS
 * - No FTS5 virtual tables, no triggers -- generated tsvector columns auto-update
 *
 * Mirrors the D1 schema.ts pattern: exports DDL as a function that applies
 * all statements to a given pool.
 */
...
/**
 * Complete Postgres DDL statements.
 * Each string is a single executable statement.
 */
...
/**
 * Drop all Strata tables from the database.
 * Used in tests for clean setup/teardown.
 */
