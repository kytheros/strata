...
describe("SqliteSearchEngine integration", () => {
  ...
  it("should find results for keyword queries", async () => {
  ...
  it("should support project filtering via inline syntax", async () => {
    const results = await engine.search("project:my-app docker");
    for (const r of results) {
      expect(r.project).toBe("my-app");
    }
  });

  it("should support project filtering via options", async () => {
    const results = await engine.search("docker", { project: "my-app" });
    for (const r of results) {
      expect(r.project).toBe("my-app");
    }
  });
  ...
  it("should respect limit parameter", async () => {
    const results = await engine.search("docker", { limit: 1 });
    expect(results.length).toBeLessThanOrEqual(1);
  });

  it("should return empty for no-match queries", async () => {
    const results = await engine.search("xyznonexistentterm");
  ...
  it("should deduplicate by session (best per session)", async () => {
  ...
  it("should find solutions with solution-biased ranking", async () => {
    const results = await engine.searchSolutions("migration failed");
    expect(results.length).toBeGreaterThan(0);
    // Result about migration fix should be ranked highly
    expect(results.some((r) => r.text.toLowerCase().includes("solved"))).toBe(true);
  });

  it("should boost solution results containing fix language", async () => {
    const results = await engine.searchSolutions("error");
