...
_AGENT_SYSTEM_PROMPTS: dict[str, str] = {
    ...
    "preference": (
        ...
        "reflect the user's known preferences, interests, and context. This is "
        "NOT a fact-lookup question -- the answer is NOT directly stated in any "
        "session. You must DISCOVER what the user cares about, then TRANSFER "
        "those preferences to the question's domain.\n\n"
        "KEY INSIGHT: the user's profile vocabulary is usually DIFFERENT from "
        "the question's vocabulary. A question about 'publications' may need "
        "a user profile about 'medical image analysis research'. A question "
        "about 'a hotel in Miami' may need a user profile about 'hotels with "
        "rooftop pools'. Do NOT assume the question's words appear in the "
        "user's history.\n\n"
        "MANDATORY 4-STEP SEARCH PROTOCOL. Skipping steps is the #1 failure "
        "mode on these questions.\n\n"
        "STEP 1 -- INTEREST DISCOVERY (always first). Before searching the "
        "question's literal words, discover what the user broadly cares about. "
        "Pick 1-2 anchor queries that match the question's domain:\n"
        "  - Research/publications/learning/academic topic -> "
        "search_history('user is researching'), search_history('user field')\n"
        "  - Recommendation/hobby/travel/product topic -> "
        "search_history('user likes prefers'), search_history('user enjoys')\n"
        "  - Profession/work/tools topic -> "
        "search_history('user works on'), search_history('user uses owns')\n"
        "  - None fit cleanly -> search_history('user is') as a universal "
        "fallback.\n"
        "This step BUILDS A PROFILE of the user -- note the fields, "
        "activities, and specific items that come up repeatedly.\n\n"
        "STEP 2 -- LITERAL QUESTION SEARCH. Now search for the question's "
        "literal nouns (e.g. 'hotel Miami', 'photography setup', 'camera'). "
        "This catches cases where the user wrote about the exact topic.\n\n"
        "STEP 3 -- PARAPHRASE SEMANTIC SEARCH. Combine what you learned in "
        "Step 1 with the question intent and semantic_search a first-person "
        "paraphrase (e.g. if Step 1 revealed medical-AI research and the "
        "question asks about publications, semantic_search 'medical image "
        "analysis research papers'). The user describes things in "
        "first-person -- search for how they would have said it.\n\n"
        "STEP 4 -- OPTIONAL FOLLOW-UP. If earlier steps surfaced specific "
        "brands/products/fields, you may run a 4th search to get more "
        "detail on those items (e.g. 'Sony A7R IV', 'Edgewater Hotel', "
        "'explainable AI').\n\n"
        "SYNTHESIS. When you answer, you MUST transfer the user's discovered "
        "interests/profile (Step 1) to the question's domain.\n\n"
        "DOMINANT INTEREST RULE. The user likely has MULTIPLE overlapping "
        "interests. If Step 1 surfaced several related topics (e.g. 'deep "
        "learning for medical imaging' AND 'explainable AI' AND unrelated "
        "threads like 'nanotransducers'), you MUST identify the ONE topic "
        "that appears MOST FREQUENTLY in the Step 1 results and CENTER "
        "your answer on that dominant interest. Do not give equal weight "
        "to all discovered interests. Narrower sub-topics (e.g. "
        "'explainable AI' as a sub-area of 'deep learning for medical "
        "image analysis') are secondary context, not the main answer. "
        "Unrelated research threads (e.g. nanotransducers when the "
        "dominant medical-imaging thread is present) should be IGNORED "
        "entirely, not mixed in.\n\n"
        "EXACT VOCABULARY RULE. Use the user's EXACT phrasing from Step 1 "
        "results verbatim. If a Step 1 result says 'deep learning for "
        "medical image analysis', use that exact phrase in your answer. "
        "Do NOT paraphrase to 'XAI', 'AI in healthcare', or any synonym. "
        "The user's chosen words are the most precise signal of what they "
        "care about and are what the answer will be judged against. When "
        "recommending conferences or resources, prefer those whose names "
        "match the user's vocabulary (e.g. for a user who researches "
        "'deep learning for medical image analysis', MIDL -- 'Medical "
        "Imaging with Deep Learning' -- is a stronger match than a "
        "general imaging conference like MIUA).\n\n"
        "Examples:\n"
        "  - User researches deep learning for medical image analysis + "
        "question asks about publications -> recommend medical-imaging "
        "deep-learning publications, using the exact phrase 'deep learning "
        "for medical image analysis' in your answer, and favor conferences "
        "like MIDL whose names anchor to 'deep learning'.\n"
        "  - User likes hotels with rooftop pools + question asks about "
        "Miami hotels -> recommend Miami hotels with rooftop pools, using "
        "the phrase 'rooftop pool' explicitly.\n"
        "  - User shoots with Sony A7R IV + question asks about lenses -> "
        "recommend Sony-compatible lenses, naming 'Sony A7R IV' explicitly.\n\n"
        "NEVER answer 'I couldn't find specific details' -- if Step 1 "
        "produced ANY user context, use the dominant interest from it."
    ),
}
