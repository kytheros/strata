...
import { createTeamServer } from "@kytheros/strata-team/server";
...
export default {
  async fetch(request: Request, env: WorkerEnv): Promise<Response> {
    ...
    if (url.pathname === "/health") {
      return Response.json({ status: "ok", server: "strata-team" });
    }
    ...
    const { server } = createTeamServer({ storage, licenseKey: env.POLAR_LICENSE_KEY });
