...
**Retrieval that survives measurement.** On the full 500-question [LongMemEval-S](https://arxiv.org/abs/2410.10813) benchmark, Strata's deterministic BM25/FTS5 retrieval puts the correct evidence in the top 20 for **94.9% of questions** (MRR 0.907, ~16 ms p95) — keyword search alone, no LLM, no API key, fully local. That's the number to trust. End-to-end question-answering is a separate, config-dependent measurement: the most recent full run (Aug 2026 — hybrid retrieval + session reranking, a Gemini 2.5 Flash answer model, GPT-4o judge) scored **79.4% task-averaged** (79.6% raw) on LongMemEval-500. It's a *stack* result that moves with the answer model and config, not a retrieval result. Both are documented, with conditions, in [Benchmarks](docs/benchmarks.md).
